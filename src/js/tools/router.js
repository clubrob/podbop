(function(factory) {
  ...
  function Router(options) {
    ...
    this.mode =
      !window.history || !window.history.pushState ? 'hash' : settings.mode;
    this.root =
      settings.root === '/'
        ? '/'
        : '/' + this._trimSlashes(settings.root) + '/';
  ...
  Router.prototype._unloadCallback = function(asyncRequest) {
    ...
    if (
      ...
      result = this._currentPage.options.unloadCb(
        this._currentPage,
        asyncRequest
      );
